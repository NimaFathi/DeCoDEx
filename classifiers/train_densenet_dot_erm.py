...
class AugmentedMD(Dataset):
	def __init__(self, root_dir, random_flip=True, random_crop=True, normalize=True, image_size=256, label_name='cf pred'):
		"""
		Args:
	...
	def _load_samples(self):
		...
		for cc_icc_folder in ['CC', ]:
			for ccf_icf_folder in ['CCF', ]:
				for cd_id_folder in ['CD',]:
	...
	def __getitem__(self, idx):
		...
		with open(info_path, 'r') as f:
			for line in f:
				if line.startswith(self.label_name+':'):
...
if __name__ == "__main__":
	...
	if args.augment:
		...
		args.model_path = "/usr/local/data/nimafh/midl2024-cfdiffusion/pretrained/exp2_DOT_classification_ERM_augmented_baseline.pth"
	else:
		args.model_path = "/usr/local/data/nimafh/midl2024-cfdiffusion/pretrained/exp2_DOT_classification_ERM.pth"
	# Device
	...
	dataset_train_real = PE90DotNoSupportDataset(image_size=args.image_size,
	...
	if args.augment:
		dataset_train_augmet1 = AugmentedMD(root_dir='/usr/local/data/nimafh/midl2024-cfdiffusion/final_results/Results/exp/Dot_Baseline_result',
										random_crop=True, random_flip=True, normalize=True)
		# dataset_train_augmet2 = AugmentedMD(root_dir='/usr/local/data/nimafh/midl2024-cfdiffusion/final_results/experiment2/Results/Dot_exp2_gradreversal_basic',
		# 								random_crop=True, random_flip=True, normalize=True)
		# dataset_train_augmet3 = AugmentedMD(root_dir='/usr/local/data/nimafh/midl2024-cfdiffusion/final_results/experiment1/Results/classifier_on_dot',
		# 								random_crop=True, random_flip=True, normalize=True, label_name='class cf pred')
		dataset_train = data.ConcatDataset([dataset_train_real, dataset_train_augmet1])
	else:
