...
@torch.enable_grad()
def adjusted_clean_class_cond_fn(x_t, y, classifier,
                        s, use_logits, classifier_threshold):
    '''
    Computes the classifier gradients for the guidance
    ...
    x_in = x_t.detach().requires_grad_(True)
    logits = classifier(x_in)
    # Adjust the logits by shiting
    adjusted_logits = logits - classifier_threshold
    
    y = y.to(adjusted_logits.device).float()
    # Select the target logits,
    # for those of target 1, we take the logits as they are (sigmoid(logits) = p(y=1 | x))
    # for those of target 0, we take the negative of the logits (sigmoid(-logits) = p(y=0 | x))
    selected = y * adjusted_logits - (1 - y) * adjusted_logits
    if use_logits:
        selected = -selected
    else:
        selected = -F.logsigmoid(selected)
...
def huber_loss(student_logits, teacher_logits, delta=1.0):
...
def smooth_l1_loss(x, y, beta=1.0):
    ...
    loss = torch.where(less_than_beta, 0.5 * diff ** 2 / beta, diff - 0.5 * beta)
    return loss
        ...
        for jdx, i in enumerate(indices):
            ...
            if (dist_grad_fn is not None) and (guided_iterations > jdx):
                grads = grads + dist_grad_fn(z_t=z_t,
                                             ...
                                             alpha_t=alpha_t,
                                             **dist_grad_kargs)
            if detect_grad_fn is not None and (guided_iterations > jdx):
                grads = grads + detect_grad_fn(x_t=x_t,
                                               **detect_grad_kargs) / alpha_t
...
def get_CEDM_iterative_sampling(use_sampling=False):
    '''
    Returns DiME's main algorithm to construct counterfactuals.
    The returned function computes x_t in a recursive way.
    Easy way to set the optional parameters into the sampling
    ...
    def p_sample_loop(diffusion,
        ...
        bool_mask = (ce_mask > inpaint).float()
        ...
        for jdx, i in enumerate(indices):
            ...
            z_t_steps.append(z_t.detach())
            if inpaint != 0:
                z_t = z_t * bool_mask + (1 - bool_mask) * diffusion.q_sample(img, t)
            # out is a dictionary with the following (self-explanatory) keys:
            # 'mean', 'variance', 'log_variance'
            out = diffusion.p_mean_variance(
                model,
...
class ImageSaver():
    ...
    def construct_directory(self):
        ...
        os.makedirs(osp.join(self.output_path, 'Results', self.exp_name, 'Original', 'Correct'), exist_ok=True, )
        os.makedirs(osp.join(self.output_path, 'Results', self.exp_name, 'Original', 'Incorrect'), exist_ok=True)

        for clst, cf, subf, subf2 in itertools.product(['CC', 'IC'],
                                                ['CCF', 'ICF'],
                                                ['CD', 'ID'],
                                                ['CF', 'Noise', 'Info', 'SM']):
    ...
    def __call__(self, imgs, cfs, noises, bkl, l_1, indexes=None, masks=None, cls_lab=None, c_pred=None, cf_pred=None, cls_target=None, det_lab=None, det_pred=None, det_target=None, cf_det_pred=None):
        for idx in range(len(imgs)):
            ...
            self.save_img(img=imgs[idx],
                          ...
                          noise=noises[idx],
                          idx=current_idx,
                          cls_target=cls_target[idx].item(),
                          cls_lab=cls_lab[idx].item(),
                          c_pred=c_pred[idx].item(),
                          cf_pred=cf_pred[idx].item(),
                          det_lab=det_lab[idx].item(),
                          det_pred=det_pred[idx].item(),
                          det_target=det_target[idx].item(),
                          cf_det_pred=cf_det_pred[idx].item(),
                          bkl=bkl[idx].item(),
                          l_1=l_1[idx].item(),
                          mask=mask)
    ...
    @staticmethod
    def select_folder(label, target, pred, pred_cf, pred_det, target_det):
        folder = osp.join('CC' if label == pred else 'IC',
                          'CCF' if target == pred_cf else 'ICF', 'CD' if target_det == pred_det else 'ID')
        return folder
    ...
    def save_img(self, img, cf, noise, idx, bkl, l_1, mask, cls_lab=None, c_pred=None, cf_pred=None, cls_target=None, det_lab=None, det_pred=None, det_target=None, cf_det_pred=None):
        folder = self.select_folder(cls_lab, cls_target, c_pred, cf_pred, cf_det_pred, det_target)
        output_path = osp.join(self.output_path, 'Results',
                               self.exp_name, folder)
        img_name = f'{idx}'.zfill(7)
        orig_path = osp.join(self.output_path, 'Results', self.exp_name, 'Original',
                             'Correct' if cls_lab == c_pred else 'Incorrect',
        ...
        to_write = (f'det label: {det_lab}' +
                    f'\ndet pred (org img): {det_pred}' +
                    f'\ndet target: {det_target}'+
                    f'\ncf det pred: {cf_det_pred}' +
                    f'\nclass label: {cls_lab}' +
                    f'\nclass pred (org img): {c_pred}' +
                    f'\nclass target: {cls_target}' + 
                    f'\ncf pred: {cf_pred}' +
                    f'\nBKL: {bkl}' +
                    f'\nl_1: {l_1}')
...
class ImageSaverMultiGeneration():
    def __init__(self, output_path, exp_name, extention='.jpg'):
        self.output_path = output_path
        self.exp_name = exp_name
        self.idx = 0
    ...
    def __call__(self, imgs, cfs, noises, bkl, l_1, indexes=None, masks=None, cls_lab=None, c_pred=None, cf_pred=None, cls_target=None, det_lab=None, det_pred=None, det_target=None):
        for idx in range(len(imgs)):
            current_idx = indexes[idx].item() if indexes is not None else idx + self.idx
            mask = None if masks is None else masks[idx]
            self.save_img(img=imgs[idx],
    ...
    def save_img(self, img, cf, noise, idx, bkl, l_1, mask, cls_lab=None, c_pred=None, cf_pred=None, cls_target=None, det_lab=None, det_pred=None, det_target=None):
        output_path = osp.join(self.output_path, 'Results', self.exp_name, f'{idx}'.zfill(7))
        for clst in ['CF', 'Noise', 'Info', 'SM']:
            os.makedirs(osp.join(output_path, clst), exist_ok=True)

        # Count the number of files in the 'SM' folder and use it as the new index
        sm_path = osp.join(output_path, 'SM')
        existing_files = [f for f in os.listdir(sm_path) if osp.isfile(osp.join(sm_path, f))]
        img_name = str(len(existing_files)).zfill(3)  # New file name based on count

        orig_path = osp.join(self.output_path, 'Results', self.exp_name, 'Original', 'Correct' if cls_lab == c_pred else 'Incorrect', f'{idx}'.zfill(7) + self.extention)

        # Save mask or L0 norm image
        if mask is None:
            l0 = np.abs(img.astype('float') - cf.astype('float'))
            l0 = l0.sum(2, keepdims=True)
            l0 = 255 * l0 / l0.max()
            l0 = np.concatenate([l0] * img.shape[2], axis=2).astype('uint8')
            l0 = Image.fromarray(self.preprocess(l0))
            l0.save(osp.join(sm_path, img_name + self.extention))
        else:
            mask = mask.astype('uint8') * 255
            mask = Image.fromarray(mask)
            mask.save(osp.join(sm_path, img_name + self.extention))
    ...
    def save_mask(self, indexes):
        for idx in range(indexes.shape[0]):
            current_idx = indexes[idx].item() if indexes is not None else idx + self.idx
            folder_path = os.path.join(self.output_path, 'Results', self.exp_name, f'{current_idx:07d}')
            saliency_path = os.path.join(folder_path, 'SM')

            saliency_maps = []
            for filename in os.listdir(saliency_path):
                if filename.endswith('.jpg'):
                    img = Image.open(os.path.join(saliency_path, filename)).convert('RGB')
                    tensor_img = ToTensor()(img)
                    saliency_maps.append(tensor_img)

            if saliency_maps:
                stacked_saliency_maps = torch.stack(saliency_maps)
                aggregate_map = torch.mean(stacked_saliency_maps, dim=0)
                threshold_value = aggregate_map.mean()
                confidence_mask = (aggregate_map > threshold_value).float()
                confidence_mask = confidence_mask.mul(255).byte().cpu().numpy()
                mask = Image.fromarray(confidence_mask[0], mode='L')
                mask.save(os.path.join(folder_path, 'Mask.jpg'))
                # keep top 10% of the pixels
                # Flatten the mask to sort the values
                flat_mask = aggregate_map.flatten()

                # Find the threshold value for the top 10%
                threshold = np.percentile(flat_mask, 90)

                # Apply the threshold to keep top 25% values
                top_10_mask = (aggregate_map > threshold).float()
                top_10_mask = top_10_mask.mul(255).byte().cpu().numpy()

                # Save the processed mask
                mask_image = Image.fromarray(top_10_mask[0], mode='L')
                mask_image.save(os.path.join(folder_path, 'Top10Mask.jpg'))



                aggregate_map1 = aggregate_map.mul(255).byte().cpu().numpy()
                aggregate_map_img = Image.fromarray(aggregate_map1[0], mode='L')
                aggregate_map_img.save(os.path.join(folder_path, 'AggregateMap.jpg'))

                # Normalize the aggregate map for colormap application
                aggregate_map_np = aggregate_map.mul(255).byte().cpu().numpy()[0]  # Convert to numpy
                normed_map = (aggregate_map_np - aggregate_map_np.min()) / (aggregate_map_np.max() - aggregate_map_np.min())
                # Apply colormap to normalized aggregate map
                colorized_map = plt.get_cmap('coolwarm')(normed_map)[:, :, :3]  # Drop alpha channel
                top_10_mask_2d = top_10_mask[0]  # Assuming the first channel, adjust as necessary

                # Apply the 2D mask to each color channel
                for i in range(3):  # Assuming RGB channels
                    colorized_map[:, :, i][top_10_mask_2d == 0] = 1  # Set to white (1 in normalized colorspace)


                # Apply the "coolwarm" colormap
                plt.imsave(os.path.join(folder_path, 'ColorizedAggregateMap.jpg'), colorized_map, cmap='coolwarm')
...
class PerceptualLoss(nn.Module):
    def __init__(self, layer, c, pretrained=False):
        super().__init__()
        self.c = c
        if not pretrained:
            vgg19_model = vgg19(pretrained=False)
            vgg19_model.load_state_dict(torch.load('/home/nimafh/projects/def-arbeltal/nimafh/midl2024-cfdiffusion/core/vgg19-dcbb9e9d.pth'))
        else:
