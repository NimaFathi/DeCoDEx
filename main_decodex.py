...
from core.image_datasets import MedicalDevicePEDataset, PE90DotNoSupportDataset
...
def main():
    ...
    if args.dataset == 'PE90_DotNoSupport':
        dataset = PE90DotNoSupportDataset(image_size=args.image_size,
                            task='both',
                            data_dir=args.data_dir,
                            partition='test',
                            random_crop=False,
                            random_flip=False,
                            biased=False # add this to generate cf for group 1 and 2 (from majority to minority class)
                            )
    
    elif args.dataset == 'PE_Normal_MD_Balanced':
    ...
    for idx, (indexes, img, lab) in enumerate(loader):
        ...
        with torch.no_grad():
            ...
            cls_pred_cf = (logits_cf > args.classifier_threshold).long()

            logits_det = detector(cf)
            det_pred_cf = (logits_det > args.detector_threshold).long() 

            # adjusting the dimensions
            cls_pred_cf = cls_pred_cf.squeeze()
            det_pred_cf = det_pred_cf.squeeze()
            pred = pred.squeeze()
            ...
            stats['flipped'] += (det_pred_cf == target).sum().item()
            stats['cf pred'].append(det_pred_cf.detach().cpu())
        ...
        if args.save_images:
            save_imgs(imgs=I.numpy(), cfs=cf.numpy(), noises=noise_img.numpy(),
                        ...
                        c_pred=pred, cf_pred=cls_pred_cf, cls_target=target, det_lab=det_lab,
                        det_pred=md_pred, cf_det_pred=det_pred_cf, det_target=target_det_bool)
