...
def create_args():
    defaults = dict(
        ...
        reasoning_path='models/reasoning.pth',
...
def main():
    ...
    if args.dataset == 'PE90_DotNoSupport':
        dataset = PE90DotNoSupportDataset(image_size=args.image_size,
                            ...
                            random_crop=False,
                            random_flip=False,
                            biased=True # add this to generate cf for group 1 and 2 (from majority to minority class)
                            )
    ...
    detector.eval()
    reasoning = ClassificationModel(args.reasoning_path).to(dist_util.dev())
    reasoning.eval()
    # ========================================
    # Distance losses
    ...
    for idx, (indexes, img, lab) in enumerate(loader):
        ...
        if args.save_images:
            save_imgs(imgs=I.numpy(), cfs=cf.numpy(), noises=noise_img.numpy(),
                        ...
                        det_pred=md_pred, det_target=target_det_bool, cf_det_pred=pred_det)
