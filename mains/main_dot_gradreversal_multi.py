...
def main():
    ...
    specific_indices = [14, 18, 27, 53]  # Example indices

    # Repeat each index 10 times
    repeated_indices = [idx for idx in specific_indices for _ in range(10)]

    # Create a subset of the original dataset with the repeated indices
    subset = data.Subset(dataset, repeated_indices)
    print('Images on the dataset:', len(subset))

    loader = data.DataLoader(subset, batch_size=args.batch_size,
                             shuffle=False,
    ...
    save_imgs = ImageSaverMultiGeneration(args.output_path, args.exp_name, extention='.jpg') if args.save_images else None
    ...
    for idx, (indexes, img, lab) in enumerate(loader):
        ...
        with torch.no_grad():
            ...
            pred = (logits > args.classifier_threshold).long()
            md_logits = detector(img)
            md_pred = (md_logits > args.detector_threshold).long()
        ...
        transformed = transformed.repeat(args.generations_per_batch, 1)
        
        # generate multiple images per batch
        for gen_idx in range(args.generations_per_batch):
            flag=False
            curr_transformed = transformed[gen_idx]
            curr_transformed_cls = torch.zeros_like(transformed[gen_idx]).bool()
            curr_transformed_det = torch.zeros_like(transformed[gen_idx]).bool()
            for jdx, classifier_scale in enumerate(classifier_scales):
                ...
                for jjdx, detector_scale in enumerate(detector_sacles):
                    ...
                    model_kwargs = {}
                    model_kwargs['y'] = target[~curr_transformed]
                    # sample image from the noisy_img
                    cfs, xs_t_s, zs_t_s = sample_fn(
                        diffusion,
                        model_fn,
                        img[~curr_transformed, ...].shape,
                        args.start_step,
                        img[~curr_transformed, ...],
                        t,
                        z_t=noise_img[~curr_transformed, ...],
                        clip_denoised=args.clip_denoised,
                        model_kwargs=model_kwargs,
                        device=dist_util.dev(),
                        class_grad_fn=clean_class_cond_fn,
                        class_grad_kwargs={'y': target[~curr_transformed],
                        ...
                        detect_grad_kargs={'y': target_det[~curr_transformed],
                                            'detector': detector,
                                            's': detector_scale,
                                            'use_logits': args.use_logits},
                    ...
                    with torch.no_grad():
                        cfsl = classifier(cfs)
                        cfsp = (cfsl > args.classifier_threshold).bool()

                        cfdl = detector(cfs)
                        cfdp = (cfdl > args.detector_threshold).bool()
                    ...
                    cf[~curr_transformed] = cfs
                    for kdx in range(len(x_t_s)):
                        x_t_s[kdx][~curr_transformed] = xs_t_s[kdx]
                        z_t_s[kdx][~curr_transformed] = zs_t_s[kdx]
                    cfsp = cfsp.squeeze()
                    cfdp = cfdp.squeeze()
                    curr_target = target.squeeze()
                    curr_target_det_bool = target_det_bool.squeeze()
                   

                    curr_transformed_cls[~curr_transformed] = curr_target[~curr_transformed] == cfsp
                    curr_transformed_det[~curr_transformed] = curr_target_det_bool[~curr_transformed] == cfdp

                    # only keep True values in both transformed and transformed_det
                    curr_transformed[~curr_transformed] = curr_transformed_cls[~curr_transformed] & curr_transformed_det[~curr_transformed]
                    
                    if curr_transformed.float().sum().item() == curr_transformed.size(0):
                        flag=True
                        break
            ...
            with torch.no_grad():
                ...
                pred_cf = (logits_cf > args.classifier_threshold).long()

                logits_det = detector(cf)
                pred_det = (logits_det > args.detector_threshold).long() 
                ...
                saved_img = I.clone().detach()
                saved_img = (saved_img * 255).to(torch.uint8)
                saved_img = saved_img.permute(0, 2, 3, 1)
                saved_img = saved_img.contiguous().cpu()

                noise_img_saver = ((noise_img + 1) * 127.5).clamp(0, 255).to(torch.uint8)
                noise_img_saver = noise_img_saver.permute(0, 2, 3, 1)
                noise_img_saver = noise_img_saver.contiguous().cpu()
                ...
                # dists
                I_f = (saved_img.to(dtype=torch.float) / 255).view(saved_img.size(0), -1)
                cf_f = (cf.to(dtype=torch.float) / 255).view(saved_img.size(0), -1)
            ...
            if args.save_images:
                save_imgs(imgs=saved_img.numpy(), cfs=cf.numpy(), noises=noise_img_saver.numpy(),
        ...
        if args.save_images:
            save_imgs.save_mask(indexes=indexes.numpy())
