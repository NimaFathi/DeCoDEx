...
from core.classifier.densenet import SharedClassifier
...
def create_args():
    defaults = dict(
        ...
        mask_folder = "/usr/local/data/nimafh/midl2024-cfdiffusion/results/Results/exp/inpating_mask",
        csv_dir="",
        data_dir="",
        exp_name='',
...
def main():
    ...
    classifier.eval()
    detector = SharedClassifier(args.detector_path).to(dist_util.dev())
    detector.eval()
    # ========================================
    # Distance losses
    ...
    sample_fn = get_CEDM_iterative_sampling(use_sampling=args.use_sampling_on_x_t)
    ...
    for idx, (indexes, img, lab) in enumerate(loader):
        ...
        mask, dial_mask = read_mask(args.mask_folder, indexes)
        mask = mask.to(dist_util.dev())
        dial_mask = dial_mask.to(dist_util.dev())
        ...
        transformed_cls = torch.zeros_like(cls_lab).bool()
        ...
        for jdx, classifier_scale in enumerate(classifier_scales):
            ...
            for jjdx, detector_scale in enumerate(detector_sacles):
                ...
                transformed_cls[~transformed] = target[~transformed] == cfsp

                # only keep True values in both transformed and transformed_det
                transformed[~transformed] = transformed_cls[~transformed] & transformed_det[~transformed]
        ...
        print(I.shape, cf.shape, noise_img.shape)
        print(torch.max(I), torch.max(cf), torch.max(noise_img))
        print(torch.min(I), torch.min(cf), torch.min(noise_img))
        if args.save_images:
