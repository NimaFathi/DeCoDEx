...
from core.classifier.densenet import ClassificationModel, SharedDetector
...
def main():
    ...
    # we need detector here as a post-processing step to just show our point
    ...
    for idx, (indexes, img, lab) in enumerate(loader):
        ...
        with torch.no_grad():
            ...
            stats['flipped'] += (cls_pred_cf == target).sum().item()
            stats['cf pred'].append(cls_pred_cf.detach().cpu())
