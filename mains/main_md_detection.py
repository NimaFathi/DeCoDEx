...
from core.classifier.densenet import SharedDetector, SharedClassifier
...
def main():
    ...
    print('thesholds:', args.classifier_threshold, args.detector_threshold)
    ...
    for idx, (indexes, img, lab) in enumerate(loader):
        ...
        with torch.no_grad():
            ...
            cls_pred = (cls_logits > args.classifier_threshold).long()

        acc += (md_pred == det_lab).sum().item()
        n += det_lab.size(0)
        ...
        transformed = torch.zeros_like(det_lab).bool()

        for jdx, detector_scale in enumerate(detector_sacles):
            ...
            transformed[~transformed] = target[~transformed] == cfdp
        ...
        with torch.no_grad():
            ...
            cf_pred_cls = (logits_cf > args.classifier_threshold).long()

            logits_det = detector(cf)
            cf_pred_det = (logits_det > args.detector_threshold).long() 

            # adjusting the dimensions
            cf_pred_cls = cf_pred_cls.squeeze()
            cf_pred_det = cf_pred_det.squeeze()
            ...
            # add metrics
            dist_cf = torch.sigmoid(logits_det)
            ...
            stats['det flipped'] += (cf_pred_det == target).sum().item()
            stats['class cf pred'].append(cf_pred_cls.detach().cpu())
            stats['detector cf pred'].append(cf_pred_det.detach().cpu())
            stats['target'].append(target.detach().cpu())
            stats['det_label'].append(det_lab.detach().cpu())
            print("Keys of stats",stats.keys())
            print('Stats of class cf pred',stats['class cf pred'])
            print('Stats of target',stats['target'])
            print('Stats of detector label',stats['det_label'])
            print('Stats of detector cf pred',stats['detector cf pred'])
            print('Stats of det flipped',stats['det flipped'])
            
        if args.save_images:
            save_imgs(imgs=I.numpy(), cfs=cf.numpy(), noises=noise_img.numpy(),
                        bkl=bkl.numpy(), l_1=l_1, indexes=indexes.numpy(),
                        cls_lab=cls_lab, cls_target=cls_lab,
                        c_pred=cls_pred, cf_pred=cf_pred_cls, det_lab=det_lab,
                        det_pred=md_pred, cf_det_pred=cf_pred_det, det_target=target)
