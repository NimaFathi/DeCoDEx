...
def create_args():
    defaults = dict(
        ...
        # perceptual loss args
        lpips_pretrained=False,
    )
...
def main():
    ...
    if args.l_perc != 0:
        ...
        vggloss = PerceptualLoss(layer=args.l_perc_layer,
                                 c=args.l_perc, pretrained=args.lpips_pretrained).to(dist_util.dev())
        vggloss.eval()
    else:
        vggloss = None
    ...
    for idx, (indexes, img, lab) in enumerate(loader):
        ...
        I = (img / 2) + 0.5
        cls_lab = lab.to(dist_util.dev(), dtype=torch.long)
        ...
        with torch.no_grad():
            ...
            # TODO: check if sigmoid is applied in the detector
        ...
        for jdx, detector_scale in enumerate(detector_scales):
