    ...
    for idx, (indexes, img, lab) in enumerate(loader):
            ...
            for jjdx, detector_scale in enumerate(detector_sacles):
                ...
                cfs, xs_t_s, zs_t_s = sample_fn(
                    ...
                    dist_grad_kargs={'l1_loss': args.l1_loss,
                                    ...
                                    'l_perc': vggloss},
                    detect_grad_fn=clean_class_cond_fn,
                    # this is just for grad enhancement (changing detector label)
                    detect_grad_kargs={'y': target_det_bool[~transformed],
                                        'classifier': detector,
                                        's': detector_scale,
                                        'use_logits': args.use_logits},
        ...
        if args.save_images:
            save_imgs(imgs=I.numpy(), cfs=cf.numpy(), noises=noise_img.numpy(),
                        ...
                        det_pred=md_pred, cf_det_pred=pred_det, det_target=target_det_bool)
