...
from core.classifier.densenet import SharedClassifier, SharedDetector
...
def main():
    ...
    if args.dataset == 'PE_Normal_MD_Balanced':
        dataset = MedicalDevicePEDataset(image_size=args.image_size,
                            task='both',
    ...
    classifier = SharedClassifier(args.classifier_path).to(dist_util.dev())
    classifier.eval()
    detector = SharedDetector(args.detector_path).to(dist_util.dev())
    detector.eval()
    # ========================================
    # Distance losses
    ...
    for idx, (indexes, img, lab) in enumerate(loader):
        ...
        for gen_idx in range(args.generations_per_batch):
            ...
            if args.save_images:
                save_imgs(imgs=saved_img.numpy(), gen_idx=gen_idx, cfs=cf.numpy(), noises=noise_img_saver.numpy(),
