...
def main():
    ...
    if args.l_perc != 0:
        ...
        vggloss = PerceptualLoss(layer=args.l_perc_layer,
                                 c=args.l_perc, pretrained=True).to(dist_util.dev())
        vggloss.eval()
    else:
        vggloss = None
    ...
    acc = 0
    n = 0
    detector_scales = [float(x) for x in args.detector_scales.split(',')]
    ...
    for idx, (indexes, img, lab) in enumerate(loader):
        ...
        with torch.no_grad():
            ...
            pred = (md_logits > 0.0).long()
            c_logits = detector(img)
            c_pred = (c_logits > 0.0).long()           

        acc += (pred == det_lab).sum().item()
        n += det_lab.size(0)
        ...
        for jdx, det_scale in enumerate(detector_scales):
            ...
            cfs, xs_t_s, zs_t_s = sample_fn(
                ...
                class_grad_kwargs={'y': c_pred[~transformed],
                ...
                detect_grad_kargs={'y': target[~transformed],
                                    'detector': detector,
                                    's': det_scale,
                                    'use_logits': args.use_logits},
        ...
        with torch.no_grad():
            ...
            print('Label',cls_lab.shape)
            print('Pred',pred.shape)
            print('Target',target.shape)
            print('Pred_cf',pred_cf.shape)
            
        if args.save_images:
            save_imgs(I.numpy(), cf.numpy(), noise_img.numpy(),
                      target, det_lab, pred, pred_cf, 
                      bkl.numpy(), l_1, cls_lab, c_pred, indexes=indexes.numpy(),
                      )
